import boto3
from pprint import pprint
from csv import reader, writer

#317 topicos de SNS
client = boto3.client(
        "events",
        aws_access_key_id="ASIASUBKIF4AVJX27XVE",
        aws_secret_access_key="DyIlnkBZT2RzO4HzuORqKVbj+Vw7hJXIr18igxL/",
        aws_session_token=""
)
response:dict = client.list_rules()
#pprint(response)
#breakpoint()
with open("events_rules.csv", 'w') as f:
    write = writer(f, delimiter=',')
    row_to_write = []
    for rule in response["Rules"]:
        row_to_write.append(rule.get('Name'))
        row_to_write.append(rule.get('ScheduleExpression'))
        row_to_write.append(rule.get('Arn'))
        row_to_write.append(rule.get('Description'))
        row_to_write.append(rule.get('State'))
        write.writerow(row_to_write)
        row_to_write = []
    while 1:
        token = response.get('NextToken',{})
        if token:
            response:dict = client.list_rules(NextToken=token)
            for rule in response["Rules"]:
                row_to_write.append(rule.get('Name'))
                row_to_write.append(rule.get('ScheduleExpression'))
                row_to_write.append(rule.get('Arn'))
                row_to_write.append(rule.get('Description'))
                row_to_write.append(rule.get('State'))
                write.writerow(row_to_write)
                row_to_write = []
        else:
            break
